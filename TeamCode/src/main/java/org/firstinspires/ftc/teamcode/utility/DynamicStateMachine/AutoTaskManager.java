package org.firstinspires.ftc.teamcode.utility.DynamicStateMachine;
...
public class AutoTaskManager {
    ...
    public ArrayList<DriveFollowerTask> generateCurrentTaskDriveTaskList(Pose2d currentPos, TrajectoryVelocityConstraint VEL_CONSTRAINT, TrajectoryAccelerationConstraint ACCEL_CONSTRAINT){ // calculate a drive task to drive to reach the task start position
        ...
        if( currentTask.getTaskLocation().isUsingSpline() ){ // if the current task's target location wants us to get there using a spline
            driveTasks.add(new DriveFollowerTask(new TrajectoryBuilder(currentPos, VEL_CONSTRAINT, ACCEL_CONSTRAINT ) // create a trajectory starting at our current position that uses the drive constraints as limits
            ...
            driveTasks.add(new DriveFollowerTask(new TrajectoryBuilder(currentPos, VEL_CONSTRAINT, ACCEL_CONSTRAINT)
