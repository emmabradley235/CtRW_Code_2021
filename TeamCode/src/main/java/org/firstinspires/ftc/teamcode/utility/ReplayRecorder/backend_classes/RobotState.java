package org.firstinspires.ftc.teamcode.utility.ReplayRecorder.backend_classes;
...
public class RobotState {
    ...
    public static RobotState getStateBetween(RobotState firstState, RobotState secondState, double effectiveDriveTime, GamepadState gamepad1Override, GamepadState gamepad2Override){
        ...
        double fractionBetween = AdvMath.getFractionBetween(firstTimestamp, secondTimestamp, effectiveDriveTime); // shift everything such that the first timestamp is 0, then see what the current time is out of the second timestamp
        ...
        double x = AdvMath.interpolateBetween(firstPose.getX(), secondPose.getX(), fractionBetween); // shift the "line" to the origin, as though firstPose were the base, then multiply by the fraction between, then shift back
        double y = AdvMath.interpolateBetween(firstPose.getY(), secondPose.getY(), fractionBetween);
        double heading = AdvMath.interpolateBetween(firstPose.getHeading(), secondPose.getHeading(), fractionBetween);
    ...
    private RobotState getStateBetween(RobotState firstState, RobotState secondState, double effectiveDriveTime) {
        double fractionBetween = AdvMath.getFractionBetween(firstState.getTimestamp(), secondState.getTimestamp(), effectiveDriveTime);
