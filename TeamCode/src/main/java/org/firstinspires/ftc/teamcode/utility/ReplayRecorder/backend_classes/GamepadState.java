    ...
    public static GamepadState getGamepadstateBetween(GamepadState gamepad1State, GamepadState gamepad2State, double fractionBetween){
        if(gamepad1State == null || gamepad2State == null){ // don't try to interpolate if n
            ...
            return new GamepadState( gamepad1State,
                    (float)AdvMath.interpolateBetween(gamepad1State.left_stick_x, gamepad1State.left_stick_x, fractionBetween),
                    (float)AdvMath.interpolateBetween(gamepad1State.left_stick_y, gamepad1State.left_stick_y, fractionBetween),
                    (float)AdvMath.interpolateBetween(gamepad1State.right_stick_x, gamepad1State.right_stick_x, fractionBetween),
                    (float)AdvMath.interpolateBetween(gamepad1State.right_stick_y, gamepad1State.right_stick_y, fractionBetween),
                    (float)AdvMath.interpolateBetween(gamepad1State.left_trigger, gamepad1State.left_trigger, fractionBetween),
                    (float)AdvMath.interpolateBetween(gamepad1State.right_trigger, gamepad1State.right_trigger, fractionBetween)
            );
        }
    }
}
