...
public class AutoTask {
    ...
    public Trajectory getTrajectoryToTask(Pose2d currentPos, TrajectoryVelocityConstraint VEL_CONSTRAINT, TrajectoryAccelerationConstraint ACCEL_CONSTRAINT){ // a method that generates a trajectory based on current position and target drive position
        if( taskLocation.isUsingSpline() ){ // if the target position tells us to use a spline path, generate a spline path
            return new TrajectoryBuilder( currentPos, VEL_CONSTRAINT, ACCEL_CONSTRAINT ) // have the trajectory start at the passed current position and use the passed in drive constraints
