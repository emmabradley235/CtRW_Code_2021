package org.firstinspires.ftc.teamcode.hardware.mecanum_drives.ReplayRecorder;
...
@Config
public class Drive_Mecanum_RepRec {
...
//     --------------------------  CLASS START  --------------------------
    ...
    // Default constructor
    public Drive_Mecanum_RepRec(DcMotor driveMotorFL, DcMotor driveMotorFR, DcMotor driveMotorBL, DcMotor driveMotorBR){ // passing of individual motors in a constructor as an alternative to needing a robot class passed with the proper motor names
    ...
    public void driveToReplayPose(Pose2d currentPose, Pose2d targetPose){ // drives the robot to a target position when called in a loop
        ...
        driveFieldRelative(replayBaseXVelo + xVelo, replayBaseYVelo + yVelo, replayBaseHeadingVelo + headingVelo, currentPose.getHeading(), false); // then drive field relative at those velocities
    }


    public void setReplayBaseMovement(double xVelo, double yVelo, double headingVelo, double currentHeading, boolean regulatingSpeed, boolean velosFieldRelative){ // sets the proactive part of the replay movement (used for setting the original controller inputs to move, letting the PID just correct rather than do the bulk of the driving)
        ...
        if( regulatingSpeed ){
            xVelo = getLimitedSpeed(xVelo, 'X'); // limit the speeds
            yVelo = getLimitedSpeed(yVelo, 'Y');
            headingVelo = getLimitedSpeed(headingVelo, 'R');
        }
