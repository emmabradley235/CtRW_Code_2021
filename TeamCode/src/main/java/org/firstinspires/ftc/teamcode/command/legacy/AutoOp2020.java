...
import org.firstinspires.ftc.teamcode.utility.OpenCV.RingStackHeightPipeline;
...
public class AutoOp2020 extends LinearOpMode {
    ...
    public static double ARM_OFFSET_DEGREES = -365; // an offset for the wobble arm
    ...
    public void runOpMode() throws InterruptedException {
        ...
        //sdrive.setTasks(taskManager.generateCurrentTaskDriveTaskList( drive.getPoseEstimate(), drive.getDriveConstraints() )); // then set the drive to go to that task's position and do any of that auto task's drive tasks
    ...
    ArrayList<AutoTask> setupAutoTasks(){
        ...
        autoTasks.add(new AutoTask("Shoot Rings", 2, new TargetDrivePosition(shootPos.getX(), shootPos.getY(), shootPos.getHeading() - Math.toRadians(0.5)), atLocationTasks).setCompleted(true)); // set completed so that the pathing algorithm won't consider it until it is set true (which will happen once its prerequisite task becomes completed)
